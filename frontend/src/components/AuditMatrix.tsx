...
export const AuditMatrix: React.FC = () => {
  const { auditResults, auditRequirements, documents, openPreview, t } = useDocuments();
    ...
    <div className="w-full space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="p-4 rounded-xl bg-slate-900/80 border border-slate-800">
          <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">{t('auditedDocs')}</span>
        ...
        >
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">{t('passedAll')}</span>
            <ShieldCheck className="w-4 h-4 text-brand-400" />
          </div>
          <div className="mt-2 flex items-baseline gap-2">
            <span className="text-2xl font-black text-brand-400">{passedCount}</span>
            <span className="text-xs text-brand-400/80">{t('passedBadge')}</span>
          </div>
        </div>
        ...
        >
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">{t('needsReview')}</span>
            <AlertTriangle className="w-4 h-4 text-amber-400" />
          </div>
          <div className="mt-2 flex items-baseline gap-2">
            <span className="text-2xl font-black text-amber-400">{reviewCount}</span>
            <span className="text-xs text-amber-300/80">{t('reviewBadge')}</span>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {reviewCount > 0 && (
            ...
            >
              ...
              <span>{isZipping ? 'Zipping...' : t('downloadReviewZip')}</span>
            </button>
          )}
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs text-slate-300">
            <thead className="bg-slate-900/90 text-slate-400 uppercase tracking-wider font-semibold border-b border-slate-800">
              <tr>
                <th className="py-3.5 px-4 font-bold text-white min-w-[220px]">{t('docFileHeader')}</th>
